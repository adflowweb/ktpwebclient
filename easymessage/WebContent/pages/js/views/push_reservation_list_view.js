...
ADF.PushReservationListView = Backbone.View.extend({	
	events : {
	...
	initialize : function() {
		_.bindAll(this, 'render', 'beforeRender', 'afterRender','reservationCancel','reservationSearch','searchFormCheck','reservationCheckbox');
		var _this = this;
	...
	resetSearch:function(){
		$('#reservation-search-select').val("0").attr("selected","selected");
	...
	render : function() {
		...
		$.get('pages/js/template/push_reservation_list_template.html',
				function(data) {
					...
					$('#reservation-date-time-picker').datetimepicker({
						viewMode : 'years',
						format : 'YYYY/MM',
						minViewMode : "months",
						pickTime : false
					});
					reservationListTable = $('#reservation-list-table')
							.dataTable({
								'bSort' : false,
								'bServerSide' : true,
								'bFilter' : false,
												...
												success : function(data) {
													...
													if (!data.result.errors) {
														...
														for ( var i in dataResult) {
															...
															dataResult[i].content = pushUtil.b64_to_utf8(dataResult[i].content);

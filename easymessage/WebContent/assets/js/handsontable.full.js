    ...
    function diff (input, units, asFloat) {
        ...
        if (units === 'year' || units === 'month' || units === 'quarter') {
            ...
            output = units === 'second' ? delta / 1e3 : // 1000
                ...
                units === 'day' ? (delta - zoneDelta) / 864e5 : // 1000 * 60 *
																// 60 * 24,
																// negate dst
...
}(this, function (moment)
{
    ...
    defaults = {
        ...
        // automatically show/hide the picker on `field` focus (default `true`
		// if `field` is set)
        bound: undefined,

        // position of the datepicker, relative to the field (default to bottom
		// & left)
        // ('bottom' & 'left' keywords are not used, 'top' & 'right' are
		// modifier on the bottom/left position)
        position: 'bottom left',

        // automatically fit in the viewport even if it means repositioning from
		// the position option
        reposition: true,
        ...
        // only used for the first display or when a selected date is not
		// visible
        mainCalendar: 'left',
    ...
    Pikaday.prototype = {
        ...
        /**
		 * return a formatted string of the current selection (using Moment.js
		 * if available)
		 */
 ...
 * interface. Copyright (c) 2009-2014 Jon Rohan, James M. Greene Licensed MIT
 * http://zeroclipboard.org/ v2.2.0
 */
(function(window, undefined) {
  ...
  /**
	 ...
	 * based on a list of `keys` to keep. The inverse of `_omit`, mostly. The
	 * big difference is that these properties do NOT need to be enumerable to
	 * be kept.
	 * 
  ...
  /**
	 ...
	 * are either (a) inline or (b) from differing parent directories, this
	 * method must return `undefined`.
	 * 
	 * @returns String or `undefined`
	 * @private
	 */
  ...
  /**
	 ...
	 * location of the executing JavaScript file (e.g. "ZeroClipboard.js",
	 * etc.).
	 * 
	 * @returns String
	 * @private
  ...
  /**
	 * The minimum Flash Player version required to use ZeroClipboard
	 * completely.
	 * 
	 * @readonly
	 * @private
  ...
  /**
	 * The ZeroClipboard library version number, as reported by Flash, at the
	 * time the SWF was compiled.
	 */
  var _zcSwfVersion;
  /**
  ...
  var _currentElement;
  /**
	 * Keep track of the element that was activated when a `copy` process
	 * started.
	 * 
	 * @private
	 */
  ...
  /**
	 * Determine if an event's registered handlers should be execute
  ...
  /**
	 ...
	 * 
	 * IMPORTANT: This is NOT a necessary check but it can result in
	 * significantly faster detection of bad `swfPath` configuration and/or
	 * network/server issues [in supported browsers] than waiting for the entire
	 * `flashLoadTimeout` duration to elapse before detecting that the SWF
	 * cannot be loaded. The detection duration can be anywhere from 10-30 times
	 * faster [in supported browsers] by using this approach.
	 * 
	 * @returns `undefined`
	 * @private
	 */
  ...
  /**
	 * Get the HTML element container that wraps the Flash bridge
	 * object/element.
	 * 
	 * @private
	 */
  ...
  /**
	 * Map the data format names from Flash-friendly names back to their
	 * original "clipData" names (via a format mapping).
	 * 
	 * @returns A new transformed object.
	 * @private
	 */
  ...
  /**
	 ...
	 * "&noCache={time}" query param string to return. Does NOT append that
	 * string to the original path. This is useful because ExternalInterface
	 * often breaks when a Flash SWF is cached.
	 * 
  ...
  /**
	 ...
	 * iframe. If it is, Flash Player cannot be used, so ZeroClipboard is dead
	 * in the water.
	 * 
  ...
  _detectFlashSupport(_ActiveXObject);
  /**
	 * Always assess the `sandboxed` state of the page at important
	 * Flash-related moments.
	 */
  _detectSandbox(true);
  /**
  ...
  /**
	 * Check if Flash is unusable for any reason: disabled, outdated,
	 * deactivated, etc.
	 * 
	 * @returns Boolean
	 * @static
  ...
  /**
	 * Unregister an event listener. If no `listener` function/object is
	 * provided, it will unregister all listeners for the provided `eventType`.
	 * If no `eventType` is provided, it will unregister all listeners for every
	 * event type.
	 * 
	 * @returns `ZeroClipboard`
	 * @static
  ...
  /**
	 * Retrieve event listeners for an `eventType`. If no `eventType` is
	 * provided, it will retrieve all listeners for every event type.
	 * 
	 * @returns array of listeners for the `eventType`; if no `eventType`, then
	 *          a map/hash object of listeners for all event types; or `null`
	 */
  ZeroClipboard.handlers = function() {
    return _listeners.apply(this, _args(arguments));
  };
  /**
	 * Event emission receiver from the Flash object, forwarding to any
	 * registered JavaScript event listeners.
	 * 
  ...
  /**
	 * Self-destruct and clean up everything, including the embedded Flash
	 * object.
	 * 
	 * @returns `undefined`
	 * @static
  ...
  /**
	 * Clear the pending data for clipboard injection. If no `format` is
  ...
  /**
	 ...
	 * on the setup, this may also set the pending clipboard text data as well
	 * as the Flash object's wrapping element's title attribute based on the
  ...
  /**
	 ...
	 * off-screen; depending on the setup, this may also unset the Flash
	 * object's wrapping element's title attribute based on the underlying HTML
	 * element and ZeroClipboard configuration.
	 * 
	 * @returns `undefined`
	 * @static
	 */
  ...
  /**
	 * The underlying implementation of
	 * `ZeroClipboard.Client.prototype.handlers`.
	 * 
	 * @private
	 */
  ...
  /**
	 * The underlying implementation of
	 * `ZeroClipboard.Client.prototype.elements`.
	 * 
	 * @private
	 */
  ...
  /**
	 ...
	 * provided `eventType`. If no `eventType` is provided, it will unregister
	 * all handlers for every event type.
	 * 
	 * @returns `this`
	 */
  ...
  /**
	 * Event emission receiver from the Flash object for this client's
	 * registered JavaScript event listeners.
	 * 
	 * @returns For the "copy" event, returns the Flash-friendly "clipData"
	 *          object; otherwise `undefined`.
  ...
  /**
	 * Unregister the clipboard actions of previously registered element(s) on
	 * the page. If no elements are provided, ALL registered elements will be
	 * unregistered.
	 * 
	 * @returns `this`
	 */
  ...
  /**
	 ...
	 * `format` is provided, a copy of ALL pending data formats will be
	 * returned.
	 * 
	 * @returns `String` or `Object`
	 */
